...
//> DEVICE PROPERTIES	
	...
	public void run() {
		for(int baud : BAUD_RATES) {
			...
			try {
				...
				System.out.println("Problem connecting to device.");
				...
				if(serialPort != null) try { serialPort.close(); } catch(Throwable t) { 
					System.out.println("Error closing input stream.");
					t.printStackTrace(System.out);
				}
			}
		...
		finished = true;
		System.out.println("Detection completed on port: " + this.portIdentifier.getName() +
	...
	String getSerial(InputStream in, OutputStream out) throws IOException, ATDeviceDetectionException {
		...
		if(!Utils.isResponseOk(response)) {
			...
			System.out.println("Found serial: " + serial);
			if(this.serial != null) {
				// There was already a serial detected.  Check if it's the same as
				// what we've just got.
				if(!this.serial.equals(serial)) {
					System.out.println("New serial detected: '" + serial + "'.  Replacing previous: '" + this.serial + "'");
				}
			}
